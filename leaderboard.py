...
class Downloader:
    ...
    def req(self, data: str, method: str, endpoint: str) -> str:
        res = ""
        current_game_id = ""
        auth_headers = {'Authorization': 'Bearer ' + os.environ["AMGILP_DOWNLOAD_TOKEN"]}
        #print("auth_headers", auth_headers)
        with requests.request(method=method, url=endpoint, data=data, stream=True, headers=auth_headers) as r:
